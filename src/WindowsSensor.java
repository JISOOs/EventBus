...
public class WindowsSensor {
	public static void main(String args[]) {
		...
		boolean WindowsState = false; // windows state: false == 정상, true == 비정상
		float CurrentWindow;
		...
		if (em != null) {
			...
			MessageWindow mw = new MessageWindow("Windows Sensor", WinPosX, WinPosY);
			...
			CurrentWindow = GetRandomNumber() * (float) 100.00;

			mw.WriteMessage("\nInitial Windows Simulation Set::" + CurrentWindow);
			...
			while (!Done) {
				...
				PostWindow(em, CurrentWindow);

				mw.WriteMessage("Current Window::  " + CurrentWindow + " ");
				...
				for (int i = 0; i < qlen; i++) {
					...
					if (Evt.GetEventId() == -3) {
						if (Evt.GetMessage().equalsIgnoreCase("A1")) // alarm on
						{
							WindowsState = true;

						} // if

						if (Evt.GetMessage().equalsIgnoreCase("A0")) // alarm off
						{
							WindowsState = false;
	...
	static private void PostWindow(EventManagerInterface ei, float window) {
		// Here we create the event.

		Event evt = new Event((int) 3, String.valueOf(window));
