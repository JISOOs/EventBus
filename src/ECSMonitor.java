...
class ECSMonitor extends Thread {
	...
	private float windowBreak = 100;
	private float CurrentWindow = 0;
	private float DoorBreak = 100;
	private float CurrentDoor = 0;
	private float MotionDetect = 100;
	...
	public void run() {
		...
		if (em != null) {
			...
			wi = new Indicator("WIND UNK", mw.GetX() + mw.Width(), (int) (mw.Height() / 6), 3);
			di = new Indicator("DOOR UNK", mw.GetX() + mw.Width(), (int) (mw.Height()), 8);
			mi = new Indicator("MOTION UNK", mw.GetX() + mw.Width(), (int) (mw.Height() * 0.5), 8);
			...
			while (!Done) {
				...
				for (int i = 0; i < qlen; i++) {
					...
					if (Evt.GetEventId() == 3) // Window reading
					{
						try {
					...
					if (Evt.GetEventId() == 4) // Door reading
					{
						try {
					...
					if (Evt.GetEventId() == 5) // Motion reading
					{
						try {

							Hold = Float.valueOf(Evt.GetMessage()).floatValue();
				...
				if (CurrentWindow < windowBreak) {
					wi.SetLampColorAndMessage("Window Break", 3);
					WindowAlarm(ON);
				} else {

					wi.SetLampColorAndMessage("Window normal", 0);
					WindowAlarm(OFF);
				}
				if (CurrentDoor < DoorBreak) {
					di.SetLampColorAndMessage("Door Break", 3);
					DoorAlarm(ON);
				} else {

					di.SetLampColorAndMessage("Door normal", 0);
					DoorAlarm(OFF);
				}
				if (Hold < MotionDetect) {
					mi.SetLampColorAndMessage("Motion Detect", 3);
					MotionAlarm(ON);
				} else {

					mi.SetLampColorAndMessage("Hold", 0);
					MotionAlarm(OFF);
				}
	 ...
	 * range
	 *
	 * Arguments: float lowhimi - low humidity range float highhumi - high humidity
	 * range
	 *
	 * Returns: none
	 *
	...
	public void SetWindowRange(float currentWindow, float windowHIt) {
		CurrentWindow = currentWindow;
		windowBreak = windowHIt;
		mw.WriteMessage("***Window range changed to::" + CurrentWindow + "% - " + windowBreak + "%***");
	}

	public void SetDoorRange(float currentDoor, float doorHit) {
		CurrentDoor = currentDoor;
		DoorBreak = doorHit;
		mw.WriteMessage("***Door range changed to::" + CurrentDoor + "% - " + DoorBreak + "%***");

	}

	public void SetMotionRange(float currentMot, float moveMot) {
		Hold = currentMot;
		MotionDetect = moveMot;
		mw.WriteMessage("***Move range changed to::" + Hold + "% - " + MotionDetect + "%***");
	...
	/***************************************************************************
	 * CONCRETE METHOD:: Heater Purpose: This method posts events that will signal
	 * the temperature controller to turn on/off the heater
	 *
	 * Arguments: boolean ON(true)/OFF(false) - indicates whether to turn the heater
	 * on or off.
	 *
	 * Returns: none
	 *
	...
	/***************************************************************************
	 * CONCRETE METHOD:: Chiller Purpose: This method posts events that will signal
	 * the temperature controller to turn on/off the chiller
	 *
	 ...
	 *
	 ***************************************************************************/
	public void WindowAlarm(boolean ON) {
		Event evt;

		if (ON) {
			evt = new Event((int) 3, "A1");

		} else {

			evt = new Event((int) 3, "A0");
	...
	private void DoorAlarm(boolean ON) {
		Event evt;

		if (ON) {
			evt = new Event((int) 2, "D1");

		} else {

			evt = new Event((int) 2, "D0");
	...
	private void MotionAlarm(boolean ON) {
		Event evt;

		if (ON) {
			evt = new Event((int) 1, "M1");

		} else {

			evt = new Event((int) 1, "M0");
		...
		try {
			...
			System.out.println("Error sending motion detect message:: " + e);
	...
	/***************************************************************************
	 * CONCRETE METHOD:: Humidifier Purpose: This method posts events that will
	 * signal the humidity controller to turn on/off the humidifier
	 *
	 * Arguments: boolean ON(true)/OFF(false) - indicates whether to turn the
	...
	/***************************************************************************
	 * CONCRETE METHOD:: Deumidifier Purpose: This method posts events that will
	 * signal the humidity controller to turn on/off the dehumidifier
	 *
	 * Arguments: boolean ON(true)/OFF(false) - indicates whether to turn the
